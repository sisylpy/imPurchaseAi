...
Page({
  ...
  applyGoods(e) {
    ...
    var itemDis = e.currentTarget.dataset.disgoods;
    this.setData({
      placeHolder: this.data.searchString,
      index: e.currentTarget.dataset.index,
      itemDis: e.currentTarget.dataset.disgoods,
  ...
  _saveOrder: function (e) {
    ...
    //是否给weight赋值
    if (e.detail.applyStandardName == this.data.itemDis.nxDgGoodsStandardname && costPrice !== "0.1") {
  ...
  toAddGoods() {
    ...
    wx.navigateTo({
      url: '../disAddGoods/disAddGoods?goodsName=' + this.data.searchString,
    })
      ...
      queryDepDisGoodsByQuickSearchJj(data).then(res => {
        if (res.result.code == 0) {
         ...
         if(depGoodsLength == 0){
          ...
          // var comArr = res.result.data.dis;
          // var resArr = res.result.data.dep;
          // var comLength = comArr.length;
          // var resLength = resArr.length;
          // var resCom = "tabsSearch[1].amount";
          // var res = "tabsSearch[0].amount";
          // // comGoods
          // if (comArr.length > 0) {
