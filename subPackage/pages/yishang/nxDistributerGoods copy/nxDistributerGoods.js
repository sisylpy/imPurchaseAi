    ...
    nxDepGetDisCataGoodsGb(data).then(res => {
      if (res.result.code == 0) {
        ...
        this.setData({
          ...
          fatherSonsIndex: 0,
          activeSubCatId: res.result.data[0].fatherGoodsEntities[0].nxDistributerFatherGoodsId,
...
updateActiveCategory(scrollTop) {
  ...
  for (let i = 0; i < categoryPositions.length; i++) {
    if (i === categoryPositions.length - 1 || scrollTop < categoryPositions[i + 1].top) {
      activeId = categoryPositions[i].id;
      break;
    }
  }
  console.log('[联动] 当前scrollTop:', scrollTop, 'categoryPositions:', categoryPositions, '激活ID:', activeId);
  if (activeId && activeId !== this.data.activeSubCatId) {
    this.setData({ activeSubCatId: activeId });
  }
},
  ...
  _getTopFatherGoodsBack() {
    ...
    nxDepGetDisFatherGoodsByGrandIdGb(data).then(res => {
      load.hideLoading();
      if (res.result.code == 0) {
        const processedList = this.processGoodsList(res.result.data);
        this.setData({
          goodsList: processedList
        }, () => {
          this.calculateCategoryPositions();
  ...
  _getTopFatherGoods(e) {
    ...
    if (this.data.searchFather && searchId == id) {
      ...
      nxDepGetDisFatherGoodsByGrandIdGb(data).then(res => {
        ...
        if (res.result.code == 0) {
          this.setData({
            goodsList: sortAndMarkFirstInCategory(res.result.data)
          })
      ...
      nxDepGetDisFatherGoodsGb(data)
        .then((res) => {
          if (res.result.code == 0) {
            ...
            const updatedGoodsList = sortAndMarkFirstInCategory([...this.data.goodsList, ...newItems]);
...
// 排序+分组工具函数
function sortAndMarkFirstInCategory(goodsList) {
  if (!Array.isArray(goodsList)) return [];
  goodsList.sort((a, b) => {
    if (a.nxDgDfgGoodsGrandId === b.nxDgDfgGoodsGrandId) {
      return 0;
    }
  ...
  return goodsList.map(item => {
    if (item.nxDgDfgGoodsGrandId !== lastCategoryId) {
      item.isFirstInCategory = true;
      lastCategoryId = item.nxDgDfgGoodsGrandId;
    } else {
      item.isFirstInCategory = false;
    }
    return item;
  });
}