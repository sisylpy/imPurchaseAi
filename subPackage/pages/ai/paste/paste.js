...
Page({
  ...
  stopRecord() {
    ...
    var data = {
      gbNdplNxDisId: that.data.disId,
      gbNdplPaySubtotal: that.data.duration,
      gbNdplNxDepartmentFatherId: that.data.depFatherId,
      gbNdplNxDepartmentId: that.data.depId,
    };
...
,  
  ...
  getCustomerName(e){
    ...
    if(this.data.customerName == null){
      this.setData({
        customerName:  this.data.orderArr[index].gbDoGoodsName,
      })
     }
  },
  ...
  _checkOrderItemContent(order, i) {
    ...
    if (isNaN(quantityNum) || quantityNum <= 0) {
      console.warn('❌ 订单数量错误:', { quantity, quantityNum });
      wx.showModal({
        title: '订单数量错误',
  ...
  _choiceGoods() {
    ...
    if (canSave) {
      order.gbDoDisGoodsId = this.data.goodsId;
  ...
  addDisAlias(e) {
    console.log(e);
    var standard = this.data.orderArr[e.currentTarget.dataset.index].gbDoStandard;
    var name = this.data.orderArr[e.currentTarget.dataset.index].gbDoGoodsName;
    this.setData({
      orderArrIndex: e.currentTarget.dataset.index,
      customerName: name,
  ...
  downLoadGoods: function (e) {
    ...
    var dg = {
      ...
      gbDgNxGoodsId: this.data.item.gbGoodsId,
      gbDgGoodsName: this.data.item.gbGoodsName,
      gbDgNxFatherId: this.data.fatherId,
      gbDgNxFatherImg: this.data.fatherImg,
      gbDgNxFatherName: this.data.fatherName,
      gbDgGoodsDetail: this.data.item.gbGoodsDetail,
      gbDgGoodsPlace: this.data.item.gbGoodsPlace,
      gbDgGoodsBrand: this.data.item.gbGoodsBrand,
      gbDgGoodsStandardname: this.data.item.gbGoodsStandardname,
      gbDgGoodsStandardWeight: this.data.item.gbGoodsStandardWeight,
      gbDgGoodsPinyin: this.data.item.gbGoodsPinyin,
      gbDgGoodsPy: this.data.item.gbGoodsPy,
      gbDgPullOff: 0,
      gbDgGoodsStatus: 0,
      gbDgPurchaseAuto: this.data.purchaseAuto,
      gbDgNxGoodsFatherColor: this.data.color,
      gbStandardEntities: this.data.item.gbGoodsStandardEntities,
      gbAliasEntities: this.data.item.gbAliasEntities,
      gbDgPurchaseAuto: 1,
    };
    ...
    downDisGoods(dg)
      .then(res => {
        if (res.result.code == 0) {
          load.hideLoading();
          that.setData({
            goodsId: res.result.data.gbDistributerGoodsId,
            name: res.result.data.gbDgGoodsName,
          })
          that._choiceGoods()
  ...
  _updateDisOrder(e) {
    ...
    updateOrder(dg).then(res => {
      ...
      if (res.result.code == 0) {
        load.hideLoading();
        var goodsName = this.data.orderArr[this.data.orderArrIndex].gbDoGoodsName;
        var data = "orderArr[" + this.data.orderArrIndex + "]";
        var dataName = "orderArr[" + this.data.orderArrIndex + "].gbDoGoodsName";
        this.setData({
          [data]: res.result.data,
          [dataName]: goodsName,
        })
  ...
  addNewOrderBefore(e) {
    ...
    wx.navigateTo({
      ...
      url: '../resGoodsList/resGoodsList?depFatherId=' + this.data.depFatherId +
        ...
        '&gbDepFatherId=-1&resFatherId=-1&depSettleType=' + this.data.depInfo.gbDepartmentSettleType +
        '&beforeId=' + this.data.applyItem.gbDepartmentOrdersId
    })
  ...
  delApply() {
    ...
    var that = this;
    deleteOrder(this.data.applyItem.gbDepartmentOrdersId).then(res => {
      if (res.result.code == 0) {
