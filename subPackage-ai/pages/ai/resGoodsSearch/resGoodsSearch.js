...
Page({
  ...
  toEditApplyDep(e) {
    console.log("=== toEditApplyDep 被调用 ===");
    console.log("传递的gbDepartmentDisGoodsId:", e.currentTarget.dataset.depgoodsid);
    console.log("传递的goods对象:", e.currentTarget.dataset.depgoods);
    console.log("goods对象的gbDepartmentDisGoodsId字段:", e.currentTarget.dataset.depgoods?.gbDepartmentDisGoodsId);
    var applyItem = e.currentTarget.dataset.order;
    var itemStatus = applyItem.gbDoBuyStatus;
    console.log("订单状态 itemStatus:", itemStatus);
    if (itemStatus > 1) {
      wx.showModal({
        title: "不能修改",
      ...
      const setDataObj = {
        ...
        depgoods: e.currentTarget.dataset.depgoods,
        gbDepartmentDisGoodsId: e.currentTarget.dataset.depgoodsid,
      };
      
      // 只有当gbDepartmentDisGoodsId有值时才设置
      if (e.currentTarget.dataset.gbDepartmentDisGoodsId) {
        setDataObj.gbDepartmentDisGoodsId = e.currentTarget.dataset.gbDepartmentDisGoodsId;
      }
      
      this.setData(setDataObj);
      console.log("设置后的gbDepartmentDisGoodsId:", e.currentTarget.dataset.depgoodsid)
  ...
  _saveOrder: function (e) {
    ...
    if (this.data.itemDis !== null) {
      ...
      goodsName = this.data.itemDis.gbDgGoodsName;
      //是否给weight赋值
      ...
      goodsName = this.data.nxGoods.nxGoodsName;
    ...
    if (this.data.itemDis == null) {
      ...
      saveOrdersGbJjAndSaveGoods(dg).then(res => {
        ...
        if (res.result.code == 0) {
            ...
            var nxGoodsId = -1;
            if(res.result.data.gbDoNxGoodsId !== null){
              nxGoodsId = res.result.data.gbDoNxGoodsId;
            }
            const newOrderInfo = {
              gbDepartmentDisGoodsId: res.result.data.gbDoDepDisGoodsId,
              nxGoodsId: nxGoodsId,
              gbDepartmentOrdersEntity: res.result.data
            };
            ...
            wx.setStorageSync('needRefreshOrderData', true);
              ...
              if (dg.stockIsZero) {
                const updatedGoodsInfo = {
                  gbDepartmentDisGoodsId: res.result.data.gbDoDepDisGoodsId,
                  updatedGoodsData: res.result.data.disGoods || res.result.data
                };
                wx.setStorageSync('updatedGoodsInfo', updatedGoodsInfo);
                wx.setStorageSync('needRefreshGoodsData', true);
              }
        ...
        saveOrdersGbJjAndSaveDepGoods(dg).then(res => {
          if (res.result.code == 0) {
           load.hideLoading();
              ...
              //如果清库存，则标记需要清零库存
              console.log("dg.stockIsZero:", dg.stockIsZero)
              if (dg.stockIsZero) {
                const clearStockInfo = {
                  gbDepartmentDisGoodsId: res.result.data.gbDoDepDisGoodsId,
                  needClearStock: true
                };
                console.log("缓存清库存信息:", clearStockInfo)
                wx.setStorageSync('clearStockInfo', clearStockInfo);
                console.log("清库存缓存完成")
              }
  ...
  _updateDisOrder(e) {
    ...
    updateOrderGbJj(dg).then(res => {
      ...
      if (res.result.code == 0) {
        ...
        // 缓存修改订单信息，用于返回resGoodsAi页面时更新数据
        console.log("当前data中的gbDepartmentDisGoodsId:", that.data.gbDepartmentDisGoodsId);
        const updatedOrderInfo = {
          gbDepartmentDisGoodsId: that.data.gbDepartmentDisGoodsId || null,
          gbDepartmentOrdersEntity: res.result.data || that.data.applyItem,
          operationType: 'update'
        };
        console.log("缓存修改订单信息:", updatedOrderInfo);
        wx.setStorageSync('newOrderInfo', updatedOrderInfo);
        wx.setStorageSync('needRefreshOrderData', true);
      } else {
        load.hideLoading();
        wx.showToast({
      ...
      queryDepDisGoodsByQuickSearchGb(data).then(res => {
        if (res.result.code == 0) {
          ...
          this.setData({
            ...
            depGoodsArrAi: res.result.data.dep,
            searchResult: true,
  ...
  deleteYes() {
    ...
    deleteOrderGb(this.data.applyItem.gbDepartmentOrdersId).then(res => {
      if (res.result.code == 0) {
          ...
          if (that.data.itemIndexSearch == 0) {
           var data = "depGoodsArrAi["+ this.data.index +"].gbDepartmentOrdersEntity";
              ...
              // 缓存订单删除信息，用于返回resGoodsAi页面时更新数据
              const updatedOrderInfo = {
                gbDepartmentDisGoodsId: that.data.depGoodsArrAi[that.data.index].gbDepartmentDisGoodsId,
                gbDepartmentOrdersEntity: null,
                operationType: 'delete'
              };
              wx.setStorageSync('newOrderInfo', updatedOrderInfo);
              wx.setStorageSync('needRefreshOrderData', true);
          }
          if (that.data.itemIndexSearch == 1) {
            var data1 = "goodsList[" + this.data.index + "].gbDepartmentOrdersEntity";
            this.setData({
              [data1]: null
            })
            ...
            // 缓存订单删除信息，用于返回resGoodsAi页面时更新数据
            const updatedOrderInfo = {
              nxGoodsId: that.data.goodsList[that.data.index].nxGoodsId,
              gbDepartmentOrdersEntity: null,
              operationType: 'delete'
            };
            wx.setStorageSync('newOrderInfo', updatedOrderInfo);
  ...
  toBusiness(e) {
    ...
    wx.navigateTo({
      url: '../stockGoodsBusiness/stockGoodsBusiness?depGoodsId=' + id + '&index=' + e.currentTarget.dataset.index,
    })
  },
      ...
      saveDepProduceGoodsStock(item)
        .then(res => {
          ...
          if (res.result.code == 0) {
            ...
            // 缓存库存更新信息，用于返回resGoodsAi页面时更新数据
            const updatedGoodsInfo = {
              gbDepartmentDisGoodsId: res.result.data.gbDepartmentDisGoodsId,
              updatedGoodsData: res.result.data
            };
            wx.setStorageSync('updatedGoodsInfo', updatedGoodsInfo);
            wx.setStorageSync('needRefreshGoodsData', true);
      ...
      saveDepLossGoodsStock(item)
        .then(res => {
          ...
          if (res.result.code == 0) {
            ...
            // 缓存库存更新信息，用于返回resGoodsAi页面时更新数据
            const updatedGoodsInfo = {
              gbDepartmentDisGoodsId: res.result.data.disGoods.gbDepartmentDisGoodsId,
              updatedGoodsData: res.result.data.disGoods
            };
            wx.setStorageSync('updatedGoodsInfo', updatedGoodsInfo);
            wx.setStorageSync('needRefreshGoodsData', true);
            var that = this;
