...
Page({
  ...
  data: {
    ...
    hasShownNewGoodsAlert: false // 标记是否已显示过新商品订单提示
  ...
  onLoad: function (options) {
    ...
    // 初始化语音识别回调（使用简单直接的绑定方式）
    console.log('🔧 开始绑定语音识别回调');
    ...
    console.log('🔧 OnRecognitionStart 已绑定');

    speechRecognizerManager.OnSentenceBegin = (res) => {
      console.log('✅ 一句话开始', res)
    }
    console.log('🔧 OnSentenceBegin 已绑定');

    speechRecognizerManager.OnRecognitionResultChange = (res) => {
      console.log('🔊 识别变化时', res)
      if (res.result) {
        console.log('🔊 检测到语音内容:', res.result.voice_text_str);
        // 检测到语音，重置静音计时器
        this.setData({
        ...
        console.log('🔊 识别变化但无结果:', res);
      }
    }

    console.log('🔧 OnRecognitionResultChange 已绑定');

    speechRecognizerManager.OnSentenceEnd = (res) => {
      console.log('✅ 一句话结束', res)
    }
    console.log('🔧 OnSentenceEnd 已绑定');
    ...
    console.log('🔧 OnRecognitionComplete 已绑定');
    ...
    speechRecognizerManager.OnRecorderStop = (res) => {
      ...
      this.setData({
        ...
        isRecording: false // 确保录音状态被正确更新
      })
    }
    console.log('🔧 OnRecorderStop 已绑定');
    
    console.log('🔧 所有语音识别回调绑定完成');
  ...
  startRecord() {
    ...
    wx.getSetting({
      success: (res) => {
        console.log('🔍 麦克风权限状态:', res.authSetting);
  ...
  _startRecording() {
    ...
    console.log('[startRecord] 准备调用 speechRecognizerManager.start，参数:', params);
    console.log('[startRecord] speechRecognizerManager 状态:', speechRecognizerManager);
    
    // 添加超时检测 - 如果3秒后还没有收到OnRecognitionStart回调，说明识别服务没有启动
    const recognitionStartTimeout = setTimeout(() => {
      if (!that.data.recognitionStatus || that.data.recognitionStatus === '') {
        console.error('⚠️⚠️⚠️ 语音识别服务启动失败！3秒内未收到OnRecognitionStart回调');
        console.error('⚠️ 可能的原因:');
        console.error('⚠️ 1. 腾讯云语音识别服务异常');
        console.error('⚠️ 2. 密钥(secretkey/secretid)无效或过期');
        console.error('⚠️ 3. appid配置错误');
        console.error('⚠️ 4. 网络连接问题');
        console.error('⚠️ 5. 语音识别配额用完');
    ...
    // 在OnRecognitionStart中清除这个超时检测
    that.recognitionStartTimeout = recognitionStartTimeout;
    
    try {
      speechRecognizerManager.start(params);
      console.log('[startRecord] speechRecognizerManager.start() 调用成功');
    } catch (error) {
      console.error('[startRecord] speechRecognizerManager.start() 调用失败:', error);
  ...
  _countNewGoodsOrders() {
    console.log('🔍 _countNewGoodsOrders 开始检查');
    ...
    if (orderArr && Array.isArray(orderArr)) {
      for (let order of orderArr) {
        console.log('🔍 检查订单:', order.gbDoGoodsName, 'status:', order.gbDoStatus);
  ...
  onHide(){
    ...
    const newGoodsCount = this._countNewGoodsOrders();
    console.log('🔍 onHide - 新商品订单数量:', newGoodsCount);
